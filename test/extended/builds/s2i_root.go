...
var _ = g.Describe("[Feature:Builds][Conformance] s2i build with a root user image", func() {
	...
	g.It("should create a root build and fail without a privileged SCC", func() {
		...
		err := oc.Run("new-app").Args("docker.io/openshifttests/test-build-roots2i~https://github.com/sclorg/nodejs-ex", "--name", "nodejsfail").Execute()
	...
	g.It("should create a root build and pass with a privileged SCC", func() {
		...
		err = oc.Run("new-build").Args("docker.io/openshifttests/test-build-roots2i~https://github.com/sclorg/nodejs-ex", "--name", "nodejspass").Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
